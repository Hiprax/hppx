...
**10. hppx works with a frozen `Object.prototype`.**
Freezing `Object.prototype` (a common prototype-pollution mitigation) makes every inherited
method name read-only, and a plain assignment such as `out.toString = value` then throws. hppx
writes each key of the objects it builds as an own data property, the way `JSON.parse` does, so
`?toString=1` or `?valueOf=a&valueOf=b` are sanitized like any other key. Before 0.4.0 such a
