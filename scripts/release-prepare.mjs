#!/usr/bin/env node
// Prepare a release: bump version, ensure CHANGELOG has a heading for the
// new version, commit, push as a release branch. Stops there - you open
// the PR and merge via GitHub. After the PR merges, run release:tag to
// create and push the tag (which triggers the release workflow on CI).
//
// Usage:
...
// and the script reminds you to fill in a title before committing.
...
main(async () => {
  ...
  if (changelog) {
    ...
    if (headingRe.test(changelog)) {
      ...
      if (insertAfter > -1) {
        const updated =
          changelog.slice(0, insertAfter + 1) + "\n" + placeholder + changelog.slice(insertAfter + 1);
        await writeChangelog(updated);
      } else {
        await writeChangelog(`# Changelog\n\n${placeholder}${changelog}`);
      }
      log.ok(`CHANGELOG entry stub inserted for v${newVersion}. Edit before committing.`);
    }
  } else {
